...
export const Preferences: FC = () => {
    ...
    const checkPackageRegistryReachability = async (uuid: string, newValue: value_t) => {
        ...
        if (typeof newValue === 'string' && newValue.trim() !== '') {
            ...
            if (!result.reachable) {
                if (result.error === 'empty') {
                    toaster.warning(translate('prefs_registry_empty'), { duration: 8 })
                } else {
                    toaster.warning(translate('prefs_registry_unreachable').replace('{url}', newValue), { duration: 8 })
                }
            }
        }
    }
