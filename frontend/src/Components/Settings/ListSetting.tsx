...
export const ListSetting = ({ id, uuid, label, value, listType, onChange, createPrefComponent }) => {
    ...
    function cloneLastListEntry(currentItems: any[]) {
        const newEntry = JSON.parse(JSON.stringify(currentItems[currentItems.length - 1]))
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
                {
                    items.map((item, index) => (
                        <div key={index} style={{
                            ...
                            borderBottom: listType === 'composite' && index < items.length - 1 ? '1px solid #666' : 'none'
                        }}>
                            {items.length > 1 && listType === 'composite' && (
