...
export const DrawioEditor = ({onFloorMapAvailable}) => {
    ...
    const openDrawio = (evt) => {
        const url = 'https://embed.diagrams.net/?embed=1&ui=dark&spin=1&modified=unsavedChanges&proto=json'
    ...
    return (
        <div id='confirmationView' className='center' style={{height: 'calc(100vh - 56px)' }} onClick={(evt) => openDrawio(evt)}>
            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%'}}>
                <img id='DiagramView' className='drawio' style={{ display: previewVisible ? 'block' : 'none' }} alt={''} src={imageSrc}></img>
            </div>
        </div>
    );
