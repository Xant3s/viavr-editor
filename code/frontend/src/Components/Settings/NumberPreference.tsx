...
export const IntSetting = ({id, uuid, label, value, onChange, min, max}) => {
    return (
        <NumberSetting id={id} uuid={uuid} label={label} value={value} onChange={onChange} step={1} min={min} max={max}/>
    )
}

export const FloatSetting = ({id, uuid, label, value, onChange, min, max}) => {
    return (
        <NumberSetting id={id} uuid={uuid} label={label} value={value} onChange={onChange} step={0.1} min={min} max={max}/>
    )
}
