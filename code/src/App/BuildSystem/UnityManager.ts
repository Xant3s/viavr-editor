...
export default class UnityManager {
    ...
    constructor() {
        ipc.on('select-unity-path', async (e) => {
            const unityPath = await UnityManager.promptUserForPathToUnity()
    ...
    private async addImportedScenesToBuildSettings(projectPath: string) {
        const unityAppPath = UnityManager.isMacOS()? `${this.unityPath}/Contents/MacOS/Unity` : `${this.unityPath}`
    ...
    // @ts-ignore
    private async generateUnityProject(projectPath: string) {
        const unityAppPath = UnityManager.isMacOS()? `${this.unityPath}/Contents/MacOS/Unity` : `${this.unityPath}`
        // const generateProjectCommand = `"${unityAppPath}" -quit -batchmode -projectPath "${projectPath}" -executeMethod de.jmu.ge.BuildUtils.ProjectGenerator.GenerateProject`
        const generateProjectFilesCommand = `"${unityAppPath}" -quit -batchmode -projectPath "${projectPath}"`
        console.log('Start generating project.')
        exec(generateProjectFilesCommand, err => {
            if(err) console.log(err)
